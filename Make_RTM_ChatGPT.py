...
if __name__ == "__main__":
    ...
    assigned_orbit_indices = list(range(my_rank, len(OrbitIDs), np_mpi))
    if my_rank == 0:
        print(f"MPI size = {np_mpi}. Rank 0 will process orbits {assigned_orbit_indices}.")

    # ---------------------------------- Set up RTM -------------------------------------    surface = True      # BG: = False -> default albedo = 0     
    ...
    # Start processing (generate and read EarthCARE input files)
    for orbit_idx in assigned_orbit_indices:
        Station = StationList[orbit_idx]
        OrbitID = OrbitIDs[orbit_idx]
        ...
        for ia in ialongs:
                ...
                sza = BMAFLX.solar_zenith_angle[BMAFLX.indlatBMAFLX, 1]
                        ...
                        try:
                            ...
                            if RTM:
                                ...
                                UVS = SetRTM(
                                    ...
                                    surface=surface,
                                    aerosol=aerosol,
                                    mc_basename_path=RTInpOutPath,
                                    RTdimension=RTdimension,
                                    input_dir=RTInpOutPath,
                                ...
                                if UVS.status == 'OK':
                                    ...
                                    if UVS.inp['rte_solver'] == 'montecarlo':
                                        try:
                                            mc_flx_dir, mc_flx_dn, mc_flx_up, edir_std, edn_std, eup_std = UVspec.ReadMCOut(
                                                UVS.mc_basename, mol_abs_param=mol_abs_param
                                            )
                                            if 'AllLevels' in additional_spesifications:
                                        ...
                                        try:
                                            sza, zout, edir, edn, eup = UVspec.ReadRTOut(
                                                uvspecOutputFile, mol_abs_param=mol_abs_param
                                            )
                                        except ValueError as e:
                                            print(
                                                "Error [WORKER {}]: could not read file {} with mol_abs_param {}".
                                                format(my_rank, uvspecOutputFile, mol_abs_param)
                                            )
                                            print(f"Error [WORKER {my_rank}]: ", e)

                                        e_solar_std = e_thermal_std = None
                                        if source == 'solar':
                                            if WANT_SUR:
                                                e_solar = edir + edn
                                            else:
                                ...
                                if RTdimension == '3D':
                                    if surface:
                                        UVS.removefiles(UVS.inp['mc_albedo_type'])
                                else:
                                    print(UVS.status)
                                ...
                                edir_thermal = eup_thermal = e_thermal_std = 0.0
                ...
                if 'AllLevels' in additional_spesifications and not hasattr(e_solar, 'shape'):
                    continue
        ...
        if 'montecarlo' in rte_solver:
                if 'AllLevels' in additional_spesifications:
                    libRad.WriteNetcdf_all_levels(
                        RTOutNetcdfPath + 'libRad_' + libRad_version + '_' + src + '_' + OrbitID + '_' + Station + additional_spesifications + '.nc',
                        shape=(Nx, Ny),
                    )
                else:
                    libRad.WriteNetcdf_mc_sample_grid(
                        RTOutNetcdfPath + 'libRad_' + libRad_version + '_' + src + '_' + OrbitID + '_' + Station + additional_spesifications + '.nc',
                        shape=(Nx, Ny),
                    )
        else:
